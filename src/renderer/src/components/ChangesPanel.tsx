...
export default function ChangesPanel({ cwd, changes }: ChangesPanelProps): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    window.konductorAPI
      .getDiff(cwd, selectedFile.path, isUntracked)
      .then((raw) => {
        if (cancelled) return
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Header */}
      <div className="flex items-center px-3 py-1 bg-surface-overlay border-b border-surface-border shrink-0 min-h-[32px]">
        <span className="text-xs text-gray-400">
          Changes &middot; {changes.length} file{changes.length !== 1 ? 's' : ''}
        </span>
      </div>
